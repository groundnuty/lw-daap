...
def produce(self, fields=None):
    ...
    for field in fields:
        ...
        for value in values:
            try:
                rules = get_producer_rules(json_id,
                                           'json_for_form', 'recordext')
                for rule in rules:
                    tags = rule[0] if isinstance(
                        rule[0], tuple) else (rule[0], )

                    if tags:
                        if not any([tag in tags for tag in
                                    self.meta_metadata[field]['function']]):
                            continue
                    tmp_dict = {}

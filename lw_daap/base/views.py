    ...
    for recid in recids:
        ...
        if 'doi' in record:
            ...
            dcat += """            <dcat:Dataset rdf:about=\"""" + current_app.config[
                'CFG_SITE_URL'] + """/""" + current_app.config['CFG_SITE_RECORD'] + """/""" + str(recid) + """">\n"""
            dcat += """               <dct:identifier>""" + current_app.config['CFG_SITE_URL'] + """/""" + current_app.config[
                'CFG_SITE_RECORD'] + """/""" + str(recid) + """     </dct:identifier>\n"""

            dcat += """               <dct:title xml:lang=\"""" + \
                current_app.config['CFG_SITE_LANG'] + """\">""" + str(record['title']) + """</dct:title>\n"""
            dcat += """               <dct:description xml:lang=\"""" + current_app.config[
                'CFG_SITE_LANG'] + """\">""" + str(record['description']) + """</dct:description>\n"""
            ...
            if record['access_right'] != 'restricted' and record[
                ...
                if 'url' in record['__license_text__']:
                    dcat += """               <dct:license rdf:resource=\"""" + \
                        str((record['__license_text__'])['url']) + """\"/>\n"""

                    if 'embargo_date' in record:
                        dcat += """               <prism:embargoDate rdf:datatype="http://www.w3.org/2001/XMLSchema/#date">""" + \
                            str(record['embargo_date']) + """</prism:embargoDate>\n"""

                        for kw in record['keywords']:
                            dcat += """               <dcat:keyword>""" + \
                                str(kw) + """</dcat:keyword>\n"""

                            dcat += """               <dct:issued rdf:datatype="http://www.w3.org/2001/XMLSchema#date">""" + \
                                str(get_creation_date(recid)) + """</dct:issued>\n"""
                            dcat += """               <dct:modified rdf:datatype="http://www.w3.org/2001/XMLSchema#date">""" + \
                                str(get_modification_date(recid)) + """</dct:modified>\n"""
                            ...
                            if 'period' in record:
                                for period in record['period']:
                                    ...
                                    dcat += """                        <time:Instant>\n"""
                                    dcat += """                           <time:inXSDDateTime rdf:datatype="http://www.w3.org/2001/XMLSchema#dateTime">""" + \
                                        str(period['start']) + """</time:inXSDDateTime>\n"""
                                    ...
                                    dcat += """                        <time:Instant>\n"""
                                    dcat += """                           <time:inXSDDateTime rdf:datatype="http://www.w3.org/2001/XMLSchema#dateTime">""" + \
                                        str(period['end']) + """</time:inXSDDateTime>\n"""
                                    dcat += """                        </time:Instant>\n"""
                                    dcat += """                     </time:hasEnd>\n"""
                                    dcat += """                  </time:Interval>\n"""
                                    dcat += """               </dct:temporal>\n"""
                                    ...
                                    if 'fft' in record:
                                        for dis in record['fft']:
                                            ...
                                            if 'description' in dis:
                                                dcat += """                     <dct:title xml:lang="en">""" + \
                                                    str(dis['description']) + """</dct:title>\n"""

                                                dcat += """                     <dct:accessURL  rdf:datatype="http://www.w3.org/2001/XMLSchema#anyURI">""" + \
                                                    str(dis['url']) + """</dct:accessURL>\n"""
                                                dcat += """                     <dct:format>\n"""
                                                mimetype = str(
                                                    guess_mimetype_and_encoding(dis['url'])[0])
                                                dcat += """                        <dct:IMT rdf:value=\"""" + mimetype + \
                                                    """\" rdfs:label=\"""" + str(guess_extension(mimetype)[1:]).upper() + """\"/>\n"""
                                                dcat += """                     </dct:format>\n"""
                                                dcat += """                     <dcat:byteSize rdf:datatype="http://www.w3.org/2001/XMLSchema#integer">""" + \
                                                    str(dis['file_size']) + """</dcat:byteSize>\n"""
                                                dcat += """                  </dcat:Distribution>\n"""
                                                dcat += """               </dcat:distribution>\n"""

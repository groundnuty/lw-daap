...
def resolve_identifier(tmps, identifier):
    ...
    if is_tmp:
        if tmp_id not in tmps:
            raise Exception(
                "Temporary identifier %s not present in the dictionary" %
                (tmp_id, ))
        if tmps[tmp_id] == -1:
            # the identifier has been signalised but never assigned a value -
            # probably error during processing
            raise Exception(
                ("Temporary identifier %s has been declared, but never"
                 " assigned a value. Probably an error during processign"
                 " of an appropriate FFT has happened. Please see the log") %
                (tmp_id, ))
        return int(tmps[tmp_id])
    else:
        return int(identifier)

_re_find_001 = re.compile(
    '<controlfield\\s+tag=("001"|\'001\')\\s*>\\s*(\\d*)\\s*</controlfield>',
     re.S)
...
def bibupload(
    ...
    original_record = {}
    rec_old = {}
    # will hold record creation/modification date
    ...
    if rec_id == -1:
        ...
        write_message(
            "   -Retrieve record ID (found %s): DONE." %
            rec_id, verbose=2)
        ...
        if '001' not in record:
            ...
            write_message(
                ("   -Check if the xml marc file is already"
    ...
    if opt_mode == 'insert' or \
        ...
        write_message(
            "   -Creation of a new record id (%d): DONE" %
            rec_id, verbose=2)
        ...
        if '005' not in record:
            error = record_add_field(
                record,
                '005',
                controlfield_value=record_modification_date.strftime(
                                        "%Y%m%d%H%M%S.0"))
            if error is None:
            ...
            write_message(
                ("   Note: 005 already existing upon inserting of new"
                 " record. Keeping it."),
                verbose=2)
        record_creation_date = time.strftime(
            "%Y-%m-%d %H:%M:%S",
            time.strptime(
                record['005'][0][3].split('.')[0],
                "%Y%m%d%H%M%S"))
        ...
        record_had_altered_bit = record_get_field_values(
            rec_old,
            CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[
                :3],
        ...
        if rec_old is None:
            ...
            write_message(
                "   -Retrieve the old record to update: DONE",
                verbose=2)

        # flag to check whether the revisions have been verified and
        # patch generated.
        # If revision verification failed, then we need to manually
        # identify the affected tags
        # and process them
        ...
        if record_has_field(
                record,
                '005') and not CFG_BIBUPLOAD_DISABLE_RECORD_REVISIONS:
            write_message(
                "     -Upload Record has 005. Verifying Revision",
                verbose=2)
            try:
                rev_res = rev_verifier.verify_revision(
                    record, original_record, opt_mode)
                if rev_res:
                    opt_mode = rev_res[0]
                    record = rev_res[1]
                    ...
                    write_message(
                        lambda: ("     -Patch record generated."
                                 " Changing opt_mode to correct."
                                 " \nPatch:\n%s ") %
                        record_xml_output(record), verbose=2)
                else:
                ...
                submit_ticket_for_holding_pen(
                    rec_id,
                    err,
                    "Conflicting Revisions. Inserting record into holding pen.",
                    pretend=pretend)
                insert_record_into_holding_pen(
                    record, str(rec_id), pretend=pretend)
                ...
                submit_ticket_for_holding_pen(
                    rec_id,
                    err,
                    "Invalid Revisions. Inserting record into holding pen.",
                    pretend=pretend)
                insert_record_into_holding_pen(
                    record, str(rec_id), pretend=pretend)
                return (2, int(rec_id), msg)
                ...
                submit_ticket_for_holding_pen(
                    rec_id,
                    err,
                    "Missing 005. Inserting record into holding pen.",
                    pretend=pretend)
                insert_record_into_holding_pen(
                    record, str(rec_id), pretend=pretend)
                return (2, int(rec_id), msg)
            ...
            write_message(
                "     - No 005 Tag Present. Resuming normal flow.",
                verbose=2)
        ...
        # in case of delete operation affected tags should be deleted
        # in delete_bibrec_bibxxx
        ...
        if not revision_verified:
            ...
            write_message(
                ("     - Missing 005 or opt_mode!=Replace/Correct."
                 " Revision Verifier not called."),
                verbose=2)
            # Identify affected tags
            if (opt_mode == 'correct' or opt_mode == 'replace' or
                    opt_mode == 'replace_or_insert'):
                rec_diff=rev_verifier.compare_records(
                    record, original_record, opt_mode)
                affected_tags=rev_verifier.retrieve_affected_tags_with_ind(
                    rec_diff)
        ...
        if opt_mode == 'correct':
            ...
            write_message(
                "   -Delete the old tags to correct in the old record: DONE",
                verbose=2)
        ...
        if opt_mode == 'delete':
            ...
            for tag in existing_tags.keys():
                if tag not in retained_tags:
                    for item in existing_tags[tag]:
                        ...
                        if tag_to_add in affected_tags and (
                                ind1, ind2) not in affected_tags[tag_to_add]:
                            affected_tags[tag_to_add].append((ind1, ind2))
                        else:
                            affected_tags[tag_to_add]=[(ind1, ind2)]
                else:
                    deleted=list(
                        set(existing_tags[tag]) - set(retained_tags[tag]))
                    for item in deleted:
                        tag_to_add=item[0:3]
                        ind1, ind2=item[3], item[4]
                        if tag_to_add in affected_tags and (
            ...
            write_message(
                "   -Delete specified tags in the old record: DONE",
                verbose=2)

        # Append new tag to the old record and update the new record with the
        # old_record modified
        if opt_mode == 'append' or opt_mode == 'correct':
            record=append_new_tag_to_old_record(record, rec_old)
            write_message(
                "   -Append new tags to the old record: DONE",
                verbose=2)

        write_message(
            "     -Affected Tags found after comparing upload and original records: %s" %
            (str(affected_tags)), verbose=2)

        # 005 tag should be added everytime the record is modified
        # If an exiting record is modified, its 005 tag should be overwritten
        # with a new revision value
        if '005' in record:
            record_delete_field(record, '005')
            write_message("  Deleted the existing 005 tag.", verbose=2)
        last_revision=run_sql(
            "SELECT MAX(job_date) FROM hstRECORD WHERE id_bibrec=%s", (rec_id, ))[0][0]
        if last_revision and last_revision.strftime(
                "%Y%m%d%H%M%S.0") == record_modification_date.strftime("%Y%m%d%H%M%S.0"):
            # We are updating the same record within the same seconds! It's less than
            # the minimal granularity. Let's pause for 1 more second to take a
            # breath :-)
            time.sleep(1)
            record_modification_date=datetime.now()
        ...
        error=record_add_field(
            record,
            '005',
            controlfield_value=record_modification_date.strftime("%Y%m%d%H%M%S.0"))
        if error is None:
            write_message(
                "   Failed: Error during adding to 005 controlfield to record",
                verbose=1,
                stream=sys.stderr)
            return (1, int(rec_id))
        else:
            error=None
            write_message(lambda: "   -Added tag 005: DONE. " +
                          str(record_get_field_value(record, '005', '', '')), verbose=2)
    ...
    try:
        ...
        if record_had_FFT or extract_tag_from_record(
                record, '856') is not None:
            try:
                if bibrecdocs is None:
                    bibrecdocs=BibRecDocs(rec_id)
                record=synchronize_8564(
                    rec_id, record, record_had_FFT, bibrecdocs, pretend=pretend)
                ...
                if not insert_mode_p:  # because for insert, all tags are affected
                    ...
                    write_message(
                        "     -Modified field list updated with FFT details: %s" %
                        str(affected_tags), verbose=2)
            except Exception as e:
        ...
        write_message(
            "Stage 3: Start (Apply fields deletion requests).",
            verbose=2)
        write_message(
            lambda: "     Record before deletion:\n%s" %
            record_xml_output(record), verbose=9)
        ...
        for tag in tmp_rec:
            for data_tuple in record[tag]:
                if (CFG_BIBUPLOAD_DELETE_CODE,
                        CFG_BIBUPLOAD_DELETE_VALUE) in data_tuple[0]:
                    # delete the tag with particular indicator pairs from
                    # original record
                    record_delete_field(
                        record, tag, data_tuple[1], data_tuple[2])

        write_message(
            lambda: "     Record after cleaning up fields to be deleted:\n%s" %
            record_xml_output(record), verbose=9)

        if opt_mode == 'append':
            write_message(
                "Stage 3b: Drop duplicate fields in append mode.",
                verbose=2)
            record=record_drop_duplicate_fields(record)
            write_message(
                lambda: "     Record after dropping duplicate fields:\n%s" %
                record_xml_output(record), verbose=9)
        ...
        if updates_exist:
            ...
            if record_had_altered_bit:
                oai_provenance_fields=record_get_field_instances(
                    record, CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[
                        :3], CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[3], CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[4])
                for oai_provenance_field in oai_provenance_fields:
                    for i, (code, dummy_value) in enumerate(
                            oai_provenance_field[0]):
                        if code == CFG_OAI_PROVENANCE_ALTERED_SUBFIELD:
                            oai_provenance_field[0][i]=(code, 'true')
                            tmp_indicators=(
                                CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[
                                    3] == '_' and ' ' or CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[3],
                                CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[4] == '_' and ' ' or CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[4])
                            if tmp_indicators not in affected_tags.get(
                                    CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[:3], []):
                                if CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[
                                        :3] not in affected_tags:
                                    affected_tags[
                                        CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[
                                            :3]]=[tmp_indicators]
                                else:
                                    affected_tags[
                                        CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[
                                            :3]].append(tmp_indicators)

            write_message(
                lambda: "  Updates exists:\n%s\n!=\n%s" %
                (record, original_record), verbose=9)
            ...
            modification_date=time.strftime(
                ...
                time.strptime(
                    record_get_field_value(
                        ...
                        '005'),
                    '%Y%m%d%H%M%S.0'))
            error=update_bibfmt_format(
                rec_id, rec_xml_new, 'xm', modification_date, pretend=pretend)
            ...
            if not CFG_BIBUPLOAD_DISABLE_RECORD_REVISIONS:
                # archive MARCXML format of this record for version history
                # purposes:
                if insert_mode_p:
                    error=archive_marcxml_for_history(
                        rec_id, affected_fields={}, pretend=pretend)
                else:
                    error=archive_marcxml_for_history(
                        rec_id, affected_fields=affected_tags, pretend=pretend)
                if error == 1:
                    ...
                    write_message(
                        "   -Archived MARCXML for history: DONE", verbose=2)
        ...
        if updates_exist or record_had_FFT:
            for format_to_delete in CFG_BIBUPLOAD_DELETE_FORMATS:
                try:
                    delete_bibfmt_format(
                        rec_id, format_to_delete, pretend=pretend)
                except:
                    # OK, some formats like HB could not have been deleted, no
                    # big deal
                    pass
        write_message("   -Stage COMPLETED", verbose=2)
        ...
        write_message(
            "Stage 5: Start (Update the database with the metadata).",
            verbose=2)
        if insert_mode_p:
            update_database_with_metadata(
                record, rec_id, oai_rec_id, pretend=pretend)
            write_message("   -Stage COMPLETED", verbose=2)
            ...
            update_database_with_metadata(
                record, rec_id, oai_rec_id, affected_tags, pretend=pretend)
            write_message("   -Stage COMPLETED", verbose=2)
        else:
        ...
        write_message(
            "Stage 6: Start (Update bibrec table with current date).",
            verbose=2)
        if opt_notimechange == 0 and (updates_exist or record_had_FFT):
            record_modification_date=convert_datestruct_to_datetext(
                time.localtime())
        ...
        if record_deleted_p:
            ...
            update_database_with_metadata(
                original_record, rec_id, oai_rec_id, pretend=pretend)
            write_message(
                "   Restored original record",
                verbose=1,
                stream=sys.stderr)
...
def record_is_valid(record):
    ...
    if len(rec_ids) != 1:
        write_message(
            "    The record is not valid: it has not a single rec_id: %s" %
            (rec_ids), stream=sys.stderr)
        return False
    return True
...
def find_record_ids_by_oai_id(oaiId):
    ...
    if oaiId:
        recids=search_pattern(
            p=oaiId, f=CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG, m='e')
        ...
        if CFG_BIBUPLOAD_MATCH_DELETED_RECORDS:
            ...
            if CFG_CERN_SITE:
                return recids - (search_pattern(p='DELETED',
                                                f='980__%',
                                                m='e') | search_pattern(p='DUMMY',
                                                                        f='980__%',
                                                                        m='e'))
            else:
    ...
    def _elaborate_tag(record, tag, fun):
        if extract_tag_from_record(record, tag) is not None:
            try:
                ...
                register_exception()
                write_message(
                    "   Stage failed: ERROR: while elaborating %s tags: %s" %
                    (tag, e), verbose=1, stream=sys.stderr)
                return (1, int(rec_id))  # TODO: ?
            if record is None:
                write_message(
                    "   Stage failed: ERROR: while elaborating %s tags" %
                    (tag, ), verbose=1, stream=sys.stderr)
                return (1, int(rec_id))
            write_message("   -Stage COMPLETED", verbose=2)
        else:
            write_message("   -Stage NOT NEEDED", verbose=2)
...
def submit_ticket_for_holding_pen(rec_id, err, msg, pretend=False):
    ...
    if user:
        try:
            ...
            write_message(
                "WARNING: can't reliably retrieve uid for user %s: %s" %
                (user, err), stream=sys.stderr)
...
BibUpload task information:
    ...
    task_options: %(task_options)s""" % {
        ...
        if not pretend:
            BIBCATALOG_SYSTEM.ticket_submit(
                subject="%s: %s by %s" %
                (msg,
                 rec_id,
                 user),
                recordid=rec_id,
                text=text,
                queue=CFG_BIBUPLOAD_CONFLICTING_REVISION_TICKET_QUEUE,
                owner=uid)
...
def insert_record_into_holding_pen(record, oai_id, pretend=False):
    ...
    # here determining the identifier of the record
    bibrec_ids=find_record_ids_by_oai_id(oai_id)
    if len(bibrec_ids) > 0:
...
def open_marc_file(path):
    ...
    try:
        ...
        if erro.errno == 2:
            ...
            e=Exception('File not accessible: %s' % path)
        raise e
    return marc
...
def find_records_from_extoaiid(extoaiid, extoaisrc=None):
    ...
    assert(
        CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[
            :5] == CFG_BIBUPLOAD_EXTERNAL_OAIID_PROVENANCE_TAG[
            :5])
    bibxxx='bib' + CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[0:2] + 'x'
    bibrec_bibxxx='bibrec_' + bibxxx

    write_message(
        '   Looking for extoaiid="%s" with extoaisrc="%s"' %
        (extoaiid, extoaisrc), verbose=9)
    id_bibrecs=intbitset(
        run_sql(
            """SELECT bb.id_bibrec FROM %(bibrec_bibxxx)s AS bb,
        %(bibxxx)s AS b WHERE b.tag=%%s AND b.value=%%s
        AND bb.id_bibxxx=b.id""" % {
                'bibxxx': bibxxx,
                'bibrec_bibxxx': bibrec_bibxxx},
    ...
    write_message(
        '   Partially found %s for extoaiid="%s"' %
        (id_bibrecs, extoaiid), verbose=9)
    ...
    for id_bibrec in id_bibrecs:
        ...
        instances=record_get_field_instances(
            record,
            CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[
                0:3],
            CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[3],
        ...
        write_message(
            '   recid %s -> instances "%s"' %
            (id_bibrec, instances), verbose=9)
        for instance in instances:
            this_extoaisrc=field_get_subfield_values(
                instance, CFG_BIBUPLOAD_EXTERNAL_OAIID_PROVENANCE_TAG[5])
            this_extoaisrc=this_extoaisrc and this_extoaisrc[0] or None
            this_extoaiid=field_get_subfield_values(
                instance, CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[5])
            this_extoaiid=this_extoaiid and this_extoaiid[0] or None
            write_message(
                "        this_extoaisrc -> %s, this_extoaiid -> %s" %
                (this_extoaisrc, this_extoaiid), verbose=9)
            if this_extoaiid == extoaiid:
                write_message(
                    '   recid %s -> provenance "%s"' %
                    (id_bibrec, this_extoaisrc), verbose=9)
                if this_extoaisrc == extoaisrc:
                    write_message(
                        'Found recid %s for extoaiid="%s" with provenance="%s"' %
                        (id_bibrec, extoaiid, extoaisrc), verbose=9)
                    ret.add(id_bibrec)
                    break
                if this_extoaisrc is None:
                    write_message(
                        'WARNING: Found recid %s for extoaiid="%s" that doesn\'t specify any provenance, while input record does.' %
                        (id_bibrec, extoaiid), stream=sys.stderr)
                if extoaisrc is None:
                    write_message(
                        'WARNING: Found recid %s for extoaiid="%s" that specify a provenance (%s), while input record does not have a provenance.' %
                        (id_bibrec, extoaiid, this_extoaisrc), stream=sys.stderr)
    return ret
...
def retrieve_rec_id(record, opt_mode, pretend=False, post_phase=False):
    ...
    if rec_id is None:
        ...
        sysnos=record_get_field_values(
            record, CFG_BIBUPLOAD_EXTERNAL_SYSNO_TAG[
                0:3], CFG_BIBUPLOAD_EXTERNAL_SYSNO_TAG[
                3:4] != "_" and CFG_BIBUPLOAD_EXTERNAL_SYSNO_TAG[
    ...
    if rec_id is None:
        ...
        extoai_fields=record_get_field_instances(
            record, CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[
                0:3], CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[
                3:4] != "_" and CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[
                3:4] or "", CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[
                    4:5] != "_" and CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[
                        4:5] or "")
        if extoai_fields:
            for field in extoai_fields:
                extoaiid=field_get_subfield_values(
                    field, CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[5:6])
                extoaisrc=field_get_subfield_values(
                    field, CFG_BIBUPLOAD_EXTERNAL_OAIID_PROVENANCE_TAG[5:6])
                if extoaiid:
                    extoaiid=extoaiid[0]
                    if extoaisrc:
                        ...
                        extoaisrc=None
                    write_message(
                        "   -Checking if EXTOAIID %s (%s) exists in the database" %
                        (extoaiid, extoaisrc), verbose=9)
                    # try to find the corresponding rec id from the database
                    ...
                    if rec_ids:
                        ...
                        # OAI id later.
                        write_message(
                            "   -Tag EXTOAIID value not found in database.", verbose=9)
                        rec_id=None
        else:
            write_message(
                "   -Tag EXTOAIID not found in the xml marc file.",
                verbose=9)
    ...
    if rec_id is None:
        ...
        if record_dois:
            ...
            if len(matching_recids) > 1:
                ...
                write_message(
                    "   Failed: Multiple records found in the"
                    " database %s that match the DOI(s) in the input"
                    " MARCXML %s" %
                    (repr(matching_recids), repr(record_dois)), verbose=1, stream=sys.stderr)
                return -1
            elif len(matching_recids) == 1:
                rec_id=matching_recids.pop()
                if opt_mode == 'insert':
                    write_message(
                        "   Failed: DOI tag matching record #%s found in the xml"
                        " submitted, you should use the option replace,"
                        " correct or append to replace an existing"
                        " record. (-h for help)" %
                        rec_id, verbose=1, stream=sys.stderr)
                    return -1
            else:
    ...
    if record_dois:
        ...
        if len(matching_recids) > 1:
            ...
            if str(matching_recid) != str(rec_id):
                # Oops, this record refers to DOI existing in a different
                # record.
...
def create_new_record(rec_id=None, pretend=False):
    ...
    if rec_id is not None:
        try:
            ...
            write_message(
                "   ERROR: during the creation_new_record function: %s " %
                error, verbose=1, stream=sys.stderr)
            return None
        if run_sql("SELECT id FROM bibrec WHERE id=%s", (rec_id, )):
            write_message(
                "   ERROR: during the creation_new_record function: the requested rec_id %s already exists." %
                rec_id)
            return None
    if pretend:
        if rec_id:
    ...
    if rec_id is not None:
        return run_sql(
            "INSERT INTO bibrec (id, creation_date, modification_date) VALUES (%s, NOW(), NOW())",
            (rec_id,
             ))
    else:
        return run_sql(
...
def synchronize_8564(
    ...
    def merge_marc_into_bibdocfile(field, pretend=False):
        ...
        url=field_get_subfield_values(
            field, 'u')[
            :1] or field_get_subfield_values(
            field, 'q')[
                :1]
        ...
        if url:
            ...
            if recid != rec_id:
                write_message(
                    "INFO: URL %s is not pointing to a fulltext owned by this record (%s)" %
                    (url, recid), stream=sys.stderr)
            else:
                try:
                    ...
                    write_message(
                        "WARNING: docname %s does not seem to exist for record %s. Has it been renamed outside FFT?" %
                        (docname, recid), stream=sys.stderr)
    ...
    def merge_bibdocfile_into_marc(field, subfields):
        ...
        """
        write_message(
            'Merging subfields %s into field %s' %
            (subfields, field), verbose=9)
        subfields=dict(subfields)  # We make a copy not to have side-effects
        subfield_to_delete=[]
        for subfield_position, (code, value) in enumerate(
                field_get_subfield_instances(field)):
        ...
        subfields=sorted(subfields.items())
    ...
    write_message(
        "Synchronizing MARC of recid '%s' with:\n%s" %
        (rec_id, record), verbose=9)
    ...
    for local_position, field in enumerate(tags856s):
        if field[1] == '4' and field[2] == ' ':
            ...
            for url in field_get_subfield_values(
                    field, 'u') + field_get_subfield_values(field, 'q'):
                if url in tags8564s_to_add:
                    # there exists a link in the MARC of the record and the
                    # connection exists in BibDoc tables
                    if record_had_FFT:
                        merge_bibdocfile_into_marc(
                            field, tags8564s_to_add[url])
                    else:
                    ...
                    # or its removal. In both cases we have to remove link... a
                    # new one will be created
                    positions_tags8564s_to_remove.append(local_position)
                    write_message(
                        "%s to be deleted and re-synchronized" %
                        (field, ), verbose=9)
                    break

    record_delete_fields(record, '856', positions_tags8564s_to_remove)

    tags8564s_to_add=sorted(tags8564s_to_add.values())
        ...
        for mit in record_get_field_instances(record, 'BDM', ' ', ' '):
            ...
            if bibdoc_id is None:
                if bibdoc_name is None:
                    raise Exception(
                        "Incorrect relation. Neither name nor identifier of the first obejct has been specified")
                else:
                    ...
                    try:
                        ...
                        raise Exception(
                            "BibDoc of a name %s does not exist within a record" %
                            (bibdoc_name, ))
            else:
                if bibdoc_name is not None:
                    write_message(
                        "WARNING: both name and id of the first document of a relation have been specified. Ignoring the name",
                        stream=sys.stderr)
            if (moreinfo_str is None or mode in (
                    "replace", "correct")) and (not pretend):
...
def elaborate_brt_tags(
    ...
    # TODO: check what happens if there is no record yet ! Will the class
    ...
    if tuple_list:
        for brt in record_get_field_instances(record, 'BDR', ' ', ' '):
            ...
            if not relation_id:
                ...
                if bibdoc2_id is None:
                    if bibdoc2_name is None:
                        raise Exception(
                            "Incorrect relation. Neither name nor identifier of the second obejct has been specified")
                    else:
                        # retrieving the ID based on the document name (inside current record)
                        # The document is attached to current record.
                        try:
            ...
            if not relation_type and not relation_id:
                raise Exception("The relation type must be specified")
    ...
    if mode in ("insert", "replace_or_insert", "append", "correct", "replace"):
        ...
        if not pretend:
            for (relation_id, bibdoc1_id, bibdoc1_ver, bibdoc1_fmt,
                ...
                if more_info:
                    MoreInfo.create_from_serialised(
                        more_info, relation=relation_id)
...
def elaborate_fft_tags(record, rec_id, mode, pretend=False,
    ...
    def _add_new_format(
        ...
        write_message(
            'Add new format to %s url: %s, format: %s, docname: %s, doctype: %s, newname: %s, description: %s, comment: %s, flags: %s, modification_date: %s' %
            (repr(bibdoc),
             url,
             docformat,
             docname,
        ...
        try:
            if not url:  # Not requesting a new url. Just updating comment & description
                return _update_description_and_comment(
                    bibdoc, docname, docformat, description, comment, flags, pretend=pretend)
            try:
                if not pretend:
                ...
                write_message(
                    "('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s') not inserted because format already exists (%s)." %
                    (url,
                     docformat,
                     docname,
                     doctype,
            ...
            write_message(
                "ERROR: in adding '%s' as a new format because of: %s" %
                (url, e), stream=sys.stderr)
            raise
        return True
    ...
    def _add_new_version(
        ...
        write_message(
            'Add new version to %s url: %s, format: %s, docname: %s, doctype: %s, newname: %s, description: %s, comment: %s, flags: %s' %
            (repr(bibdoc), url, docformat, docname, doctype, newname, description, comment, flags), verbose=9)
        try:
            if not url:
                return _update_description_and_comment(
                    bibdoc, docname, docformat, description, comment, flags, pretend=pretend)
            try:
                ...
                write_message(
                    "('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s') not inserted because '%s'." %
                    (url,
                     docformat,
                     docname,
                     doctype,
            ...
            write_message(
                "ERROR: in adding '%s' as a new version because of: %s" %
                (url, e), stream=sys.stderr)
            raise
        return True

    def _update_description_and_comment(
            bibdoc,
            docname,
            docformat,
            description,
        ...
        write_message(
            'Just updating description and comment for %s with format %s with description %s, comment %s and flags %s' %
            (docname, docformat, description, comment, flags), verbose=9)
        try:
            if not pretend:
            ...
            write_message(
                "('%s', '%s', '%s', '%s', '%s') description and comment not updated because '%s'." %
                (docname, docformat, description, comment, flags, e))
            raise
        return True

    def _process_document_moreinfos(
            more_infos,
            docname,
            version,
            docformat,
        ...
        if not doc_moreinfo_s is None:
            MoreInfo.create_from_serialised(
                ser_str=doc_moreinfo_s, docid=docid)
    ...
    if mode == 'delete':
        raise Exception(
            'FFT tag specified but bibupload executed in --delete mode')
    ...
    if tuple_list:  # FFT Tags analysis
        ...
        for fft in record_get_field_instances(record, 'FFT', ' ', ' '):
            ...
            # checking if version is temporary... if so, filling a different
            # varaible
            ...
            # value -1 means that identifier has been declared but not assigned
            # a value yet
            if bibdoc_tmpid:
                if bibdoc_tmpid in tmp_ids:
                    write_message(
                        "WARNING: the temporary identifier %s has been declared more than once. Ignoring the second occurance" %
                        (bibdoc_tmpid, ), stream=sys.stderr)
                else:
            ...
            if bibdoc_tmpver:
                if bibdoc_tmpver in tmp_vers:
                    write_message(
                        "WARNING: the temporary version identifier %s has been declared more than once. Ignoring the second occurance" %
                        (bibdoc_tmpver, ), stream=sys.stderr)
                else:
            ...
            if url:
                ...
                try:
                    ...
                    raise Exception(
                        "fft '%s' specifies in $a a location ('%s') with problems: %s" %
                        (fft, url, e))
            else:
                url=''

# TODO: a lot of code can be compactified using similar syntax ... should be more readable on the longer scale
# maybe right side expressions look a bit cryptic, but the elaborate_fft
# function would be much clearer
...
#                    description = KEEP_OLD_VALUE
            ...
            if name:
                ...
                if url:
                    ...
                    raise Exception(
                        "WARNING: fft '%s' doesn't specifies either a location in $a or a docname in $n" %
                        str(fft))
                else:
                    continue

            # Let's discover the desired new docname in case we want to change
            # it
            ...
            if icon != []:
                ...
                if icon != KEEP_OLD_VALUE:
                    try:
                        ...
                        raise Exception(
                            "fft '%s' specifies in $x an icon ('%s') with problems: %s" %
                            (fft, icon, e))
            else:
                icon=''
            ...
            if timestamp:
                try:
                    timestamp=datetime(
                        *(time.strptime(timestamp[0], "%Y-%m-%d %H:%M:%S")[:6]))
                except ValueError:
                    write_message(
                        'WARNING: The timestamp is not in a good format, thus will be ignored. The format should be YYYY-MM-DD HH:MM:SS',
                        stream=sys.stderr)
                    timestamp=''
            else:
                timestamp=''
            ...
            for flag in flags:
                if flag not in CFG_BIBDOCFILE_AVAILABLE_FLAGS:
                    raise Exception(
                        "fft '%s' specifies a non available flag: %s" %
                        (fft, flag))
            ...
            if name in docs:  # new format considered
                (doctype2,
                 ...
                 version2,
                 urls,
                 dummybibdoc_moreinfos2,
                 dummybibdoc_tmpid2,
                 dummybibdoc_tmpver2)=docs[name]
                if doctype2 != doctype:
                    raise Exception(
                        "fft '%s' specifies a different doctype from previous fft with docname '%s'" %
                        (str(fft), name))
                if newname2 != newname:
                    raise Exception(
                        "fft '%s' specifies a different newname from previous fft with docname '%s'" %
                        (str(fft), name))
                if restriction2 != restriction:
                    raise Exception(
                        "fft '%s' specifies a different restriction from previous fft with docname '%s'" %
                        (str(fft), name))
                if version2 != version:
                    raise Exception(
                        "fft '%s' specifies a different version than the previous fft with docname '%s'" %
                        (str(fft), name))
                for (
                        dummyurl2,
                        format2,
                        ...
                        dummycomment2,
                        dummyflags2,
                        dummytimestamp2) in urls:
                    if docformat == format2:
                        raise Exception(
                            "fft '%s' specifies a second file '%s' with the same format '%s' from previous fft with docname '%s'" %
                            (str(fft), url, docformat, name))
                if url or docformat:
                    urls.append(
                        (url, docformat, description, comment, flags, timestamp))
                if icon:
                    urls.append((icon,
                                 icon[len(file_strip_ext(icon)):] + ';icon',
                                 description,
                                 comment,
                                 flags,
                                 timestamp))
                ...
                if url or docformat:
                    docs[name]=(doctype,
                                  ...
                                  [document_moreinfo,
                                   version_moreinfo,
                                   version_format_moreinfo,
                                   format_moreinfo],
                                  bibdoc_tmpid,
                                  bibdoc_tmpver,
                                  description)
                    if icon:
                        docs[name][4].append((icon, icon[len(file_strip_ext(
                            icon)):] + ';icon', description, comment, flags, timestamp))
                elif icon:
                    docs[name]=(doctype,
                                  newname,
                                  restriction,
        ...
        for docname, (doctype, newname, restriction, version, urls, more_infos,
                      bibdoc_tmpid, bibdoc_tmpver, description) in docs.items():
            downloaded_urls=[]
            try:
                bibdoc=bibrecdocs.get_bibdoc(docname)
            except InvenioBibDocFileError:
            ...
            for url, docformat, description, comment, flags, timestamp in urls:
                if url:
                    try:
                        ...
                        write_message(
                            "%s saved into %s" %
                            (url, downloaded_url), verbose=9)
                    except Exception as err:
                        write_message(
                            "ERROR: in downloading '%s' because of: %s" %
                            (url, err), stream=sys.stderr)
                        raise
                    if mode == 'correct' and bibdoc is not None and not new_revision_needed:
                        downloaded_urls.append(
                            (downloaded_url, docformat, description, comment, flags, timestamp))
                        if not bibrecdocs.check_file_exists(
                                downloaded_url, docformat):
                            new_revision_needed=True
                        else:
                            write_message(
                                "WARNING: %s is already attached to bibdoc %s for recid %s" %
                                (url, docname, rec_id), stream=sys.stderr)
                    ...
                    downloaded_urls.append(
                        ('', docformat, description, comment, flags, timestamp))
            if mode == 'correct' and bibdoc is not None and not new_revision_needed:
                ...
                write_message(
                    "No need to add a new revision for docname %s for recid %s" %
                    (docname, rec_id), verbose=2)
                docs[docname]=(doctype,
                                 newname,
                                 restriction,
                                 version,
                                 [('',
        ...
        for docname, (doctype, newname, restriction, version, urls, more_infos,
                      bibdoc_tmpid, bibdoc_tmpver, description) in iteritems(docs):
            write_message(
                "Elaborating olddocname: '%s', newdocname: '%s', doctype: '%s', restriction: '%s', urls: '%s', mode: '%s'" %
                (docname, newname, doctype, restriction, urls, mode), verbose=9)
            if mode in (
                    'insert',
                    'replace'):  # new bibdocs, new docnames, new marc
                if newname in bibrecdocs.get_bibdoc_names():
                    write_message(
                        "('%s', '%s') not inserted because docname already exists." %
                        (newname, urls), stream=sys.stderr)
                    raise Exception(
                        "('%s', '%s') not inserted because docname already exists." %
                        (newname, urls), stream=sys.stderr)
                try:
                    ...
                    write_message(
                        "('%s', '%s', '%s') not inserted because: '%s'." %
                        (doctype, newname, urls, e), stream=sys.stderr)
                    raise e
                for (
                        url,
                        docformat,
                ...
                try:
                    ...
                    if doctype not in (
                        ...
                        if newname != docname:
                            try:
                                if not pretend:
                                    bibrecdocs.change_name(
                                        newname=newname, docid=bibdoc.id)
                                    write_message(
                                        lambda: "After renaming: %s" %
                                        bibrecdocs, verbose=9)
                            except Exception as e:
                                write_message(
                                    'ERROR: in renaming %s to %s: %s' %
                                    (docname, newname, e), stream=sys.stderr)
                                raise
                try:
                    ...
                    if doctype == 'PURGE':
                        ...
                        try:
                            ...
                            write_message(
                                '(%s, %s) not correctly reverted: %s' %
                                (newname, version, e), stream=sys.stderr)
                            raise
                    else:
                ...
                try:
                    ...
                    if doctype == 'PURGE':
                        ...
                        if description != KEEP_OLD_VALUE:
                            if not pretend:
                                write_message(
                                    "SHOULD CHANGE DESCRIPTION %s" %
                                    (description), verbose=9)
                                bibdoc.set_description(
                                    description, bibdoc.list_latest_files()[0].format)
                ...
                if not found_bibdoc:
                    if doctype in (
                            ...
                            'REVERT'):
                        write_message(
                            "('%s', '%s', '%s') not performed because '%s' docname didn't existed." %
                            (doctype, newname, urls, docname), stream=sys.stderr)
                        raise Exception
                    else:
                        if not pretend:
                            bibdoc=bibrecdocs.add_bibdoc(doctype, newname)
                ...
                if not found_bibdoc:
                    try:
                        ...
                        register_exception()
                        write_message(
                            "('%s', '%s', '%s') not appended because: '%s'." %
                            (doctype, newname, urls, e), stream=sys.stderr)
                        raise
            if not pretend and doctype not in ('PURGE', 'DELETE', 'EXPUNGE'):
                _process_document_moreinfos(
                    more_infos, newname, version, urls and urls[0][1], mode)
                ...
                if bibdoc_tmpver in tmp_vers and tmp_vers[bibdoc_tmpver] != -1:
                    ...
                    if version is None:
                        if version:
                            ...
                            tmp_vers[bibdoc_tmpver]=bibrecdocs.get_bibdoc(
                                docname).get_latest_version()
                    else:
                        tmp_vers[bibdoc_tmpver]=version
    return record
...
def update_bibrec_date(
    ...
    write_message(
        "   -Update record creation/modification date: DONE",
        verbose=2)


def update_bibfmt_format(
        id_bibrec,
        format_value,
        format_name,
    ...
    if nb_found == 1:
        ...
        if not pretend and row_id is None:
            write_message(
                "   ERROR: during update_bibfmt_format function",
                verbose=1,
                stream=sys.stderr)
            return 1
        else:
            write_message(
                "   -Update the format %s in bibfmt: DONE" %
                format_name, verbose=2)
            return 0

    elif nb_found > 1:
        write_message(
            "   Failed: Same format %s found several time in bibfmt for the same record." %
            format_name, verbose=1, stream=sys.stderr)
        return 1
    else:
        ...
        if res is None:
            write_message(
                "   ERROR: during insert_bibfmt",
                verbose=1,
                stream=sys.stderr)
            return 1
        else:
            write_message(
                "   -Insert the format %s in bibfmt: DONE" %
                format_name, verbose=2)
            return 0
...
def delete_bibfmt_format(id_bibrec, format_name, pretend=False):
    ...
    if not pretend:
        run_sql(
            "DELETE LOW_PRIORITY FROM bibfmt WHERE id_bibrec=%s and format=%s",
            (id_bibrec,
             format_name))
    return 0
        ...
        for field in affected_fields:
            if field.isdigit():  # hack for tags from RevisionVerifier
                for ind in affected_fields[field]:
                    tmp_affected_fields[
                        (field + ind[0] + ind[1] + "%").replace(" ", "_")]=1
            else:
                pass  # future implementation for fields
        tmp_affected_fields=sorted(tmp_affected_fields.keys())
        db_affected_fields=",".join(tmp_affected_fields)
    if res and not pretend:
        run_sql(
            """INSERT INTO hstRECORD (id_bibrec, marcxml, job_id, job_name, job_person, job_date, job_details, affected_fields)
...
def update_database_with_metadata(
        record,
        rec_id,
        oai_rec_id="oai",
        affected_tags=None,
    ...
    write_message(
        "update_database_with_metadata: record=%s, rec_id=%s, oai_rec_id=%s, affected_tags=%s" %
        (record, rec_id, oai_rec_id, affected_tags), verbose=9)
    tmp_record={}
    if affected_tags:
        for tag in record.keys():
            if tag in affected_tags.keys():
                write_message(
                    "     -Tag %s found to be modified.Setting up for update" %
                    tag, verbose=9)
    ...
    for tag in tmp_record.keys():
        ...
        if tag not in CFG_BIBUPLOAD_SPECIAL_TAGS:
            ...
            # this list should contain the elements of a full tag [tag, ind1,
            # ind2, subfield_code]
            tag_list=[]
            tag_list.append(tag)
            for single_tuple in tuple_list:
                ...
                if tag in CFG_BIBUPLOAD_SPECIAL_TAGS:
                    ...
                    (table_name, bibxxx_row_id)=insert_record_bibxxx(
                        full_tag, value, pretend=pretend)
                    # print 'tname, bibrow', table_name, bibxxx_row_id;
                    if table_name is None or bibxxx_row_id is None:
                        write_message(
                            "   Failed: during insert_record_bibxxx",
                            verbose=1,
                            stream=sys.stderr)
                    # connect bibxxx and bibrec with the table bibrec_bibxxx
                    res=insert_record_bibrec_bibxxx(
                        table_name, bibxxx_row_id, datafield_number, rec_id, pretend=pretend)
                    if res is None:
                        write_message(
                            "   Failed: during insert_record_bibrec_bibxxx",
                            verbose=1,
                            stream=sys.stderr)
                else:
                    ...
                    for subfield in set(subfield_list):
                        ...
                        if table_name is None or bibxxx_row_id is None:
                            write_message(
                                ...
                                verbose=1,
                                stream=sys.stderr)
                        # connect bibxxx and bibrec with the table
                        # bibrec_bibxxx
                        res=insert_record_bibrec_bibxxx(
                            table_name, bibxxx_row_id, datafield_number, rec_id, pretend=pretend)
                        if res is None:
        ...
        if tag in CFG_BIBUPLOAD_CONTROLFIELD_TAGS:
            if tag == '001':
                ...
                for single_tuple in record[tag]:
                    ...
                    newfield_number=record_add_field(
                        rec_old, tag, controlfield_value=controlfield_value)
                    if newfield_number is None:
                        write_message(
                            "   ERROR: when adding the field" + tag,
                            verbose=1,
                            stream=sys.stderr)
        else:
            # For each tag there is a list of tuples representing datafields
            for single_tuple in record[tag]:
                ...
                ind1=single_tuple[1]
                ind2=single_tuple[2]
                if '%s%s%s' % (tag, ind1 == ' ' and '_' or ind1, ind2 == ' ' and '_' or ind2) in (
                        CFG_BIBUPLOAD_EXTERNAL_OAIID_TAG[:5], CFG_BIBUPLOAD_EXTERNAL_SYSNO_TAG[:5]):
                    # We don't want to append the external identifier
                    # if it is already existing.
                    if record_find_field(rec_old, tag, single_tuple)[
                            0] is not None:
                        write_message(
                            "      Not adding tag: %s ind1=%s ind2=%s subfields=%s: it's already there" %
                            (tag, ind1, ind2, subfield_list), verbose=9)
                        continue
                # We add the datafield to the old record
                write_message(
                    "      Adding tag: %s ind1=%s ind2=%s subfields=%s" %
                    (tag, ind1, ind2, subfield_list), verbose=9)
                newfield_number=record_add_field(
                    rec_old, tag, ind1, ind2, subfields=subfield_list)
                if newfield_number is None:
                    write_message(
                        "   ERROR: when adding the field" + tag,
                        verbose=1,
...
def copy_strong_tags_from_old_record(record, rec_old):
    ...
    for strong_tag in CFG_BIBUPLOAD_STRONG_TAGS:
        if not record_get_field_instances(
            record, strong_tag, strong_tag[
                3:4] or '%', strong_tag[
                4:5] or '%'):
            strong_tag_old_field_instances=record_get_field_instances(
                rec_old, strong_tag)
    ...
    for tag, fields in iteritems(record):
        ...
        for field in fields:
            ...
            if local_position is not None:
                record_delete_field(
                    returned_record, tag, field_position_local=local_position)
...
def delete_tags_to_correct(record, rec_old):
    ...
    for tag in CFG_BIBUPLOAD_CONTROLLED_PROVENANCE_TAGS:
        if tag[:3] in record:
            # Let's discover the provenance that will be updated
            tmp_field_instances=record_get_field_instances(
                record, tag[:3], tag[3], tag[4])
            ...
            potential_fields_to_readd=record_get_field_instances(
                rec_old, tag[:3], tag[3], tag[4])  # Let's take all the field corresponding to tag
    ...
    for tag in record:
        ...
        if tag in rec_old and tag != '001':
            # the tag does exist, so delete all record's tag+ind1+ind2
            # combinations from rec_old
            for dummy_sf_vals, ind1, ind2, dummy_cf, dummyfield_number in record[
                    tag]:
                write_message(
                    "      Delete tag: " +
                    tag +
    ...
    for tag, fields in iteritems(fields_to_readd):
        for sf_vals in fields:
            write_message("      Adding tag: " +
                          ...
                          tag[4] +
                          " code=" +
                          str(sf_vals), verbose=9)
            record_add_field(
                rec_old, tag[
                    :3], tag[3], tag[4], subfields=sf_vals)
...
def delete_bibrec_bibxxx(record, id_bibrec, affected_tags={}, pretend=False):
    ...
    write_message(
        lambda: "delete_bibrec_bibxxx(record=%s, id_bibrec=%s, affected_tags=%s)" %
        (record, id_bibrec, affected_tags), verbose=9)
    for tag in affected_tags:
        ...
        if tag not in CFG_BIBUPLOAD_SPECIAL_TAGS:
            ...
            # delete all the records with proper id_bibrec. Indicators matter
            # for individual affected tags
            tmp_ind_1=''
            tmp_ind_2=''
            ...
            for ind_pair in affected_tags[tag]:
                ...
                # need to escape incase of underscore so that mysql treats it
                # as a char
                tag_val=tag + "\\" + tmp_ind_1 + "\\" + tmp_ind_2 + '%'
                query="""DELETE br.* FROM `%s` br,`%s` b where br.id_bibrec=%%s and br.id_bibxxx=b.id and b.tag like %%s""" % (
                    bibrec_table, table_name)
...
def task_submit_elaborate_specific_parameter(key, value, opts, args):  # pylint: disable=W0613
    ...
    # Deprecated reference mode option (now correct)
        ...
        if value.lower() in CFG_BIBUPLOAD_ALLOWED_SPECIAL_TREATMENTS:
            ...
            print(
                """The specified value is not in the list of allowed special treatments codes: %s""" %
                CFG_BIBUPLOAD_ALLOWED_SPECIAL_TREATMENTS,
                file=sys.stderr)
            return False
    elif key in ("-S", "--stage"):
        print(
            """WARNING: the --stage parameter is deprecated and ignored.""",
            file=sys.stderr)
    else:
        return False
    return True
...
def writing_rights_p():
    ...
    try:
        ...
        fd, filename=tempfile.mkstemp(
            suffix='.txt', prefix='test', dir=CFG_BIBDOCFILE_FILEDIR)
        test=os.fdopen(fd, 'w')
        test.write('TEST')
        test.close()
        if open(filename).read() != 'TEST':
            raise IOError(
                "Can not successfully write and readback %s" %
                filename)
        os.remove(filename)
    except:
        register_exception(alert_admin=True)
...
def post_results_to_callback_url(results, callback_url):
    ...
    scheme, dummynetloc, dummypath, dummyquery, dummyfragment=urlparse.urlsplit(
        callback_url)
    # See:
    ...
    if scheme == 'http':
        ...
        raise ValueError(
            "Scheme not handled %s for callback_url %s" %
            (scheme, callback_url))
    if task_get_option('oracle_friendly'):
        write_message("Oracle friendly mode requested", verbose=9)
        request=urllib2.Request(
            callback_url, data=urllib.urlencode({'results': json_results}))
    ...
    msg=res.read()
    write_message(
        "Result of posting the feedback: %s %s" %
        (res.code, res.msg), verbose=9)
    write_message("Returned message is: %s" % msg, verbose=9)
    return res


def bibupload_records(
        records,
        opt_mode=None,
        opt_notimechange=0,
        pretend=False,
        callback_url=None,
        results_for_callback=None):
    ...
    for record in records:
        ...
        if opt_mode == "holdingpen":
            ...
            if error[0] == 1:
                ...
                stat['nb_errors'] += 1
                if callback_url:
                    results_for_callback['results'].append(
                        {'recid': error[1], 'success': False, 'error_message': error[2]})
            elif error[0] == 2:
                if record:
                    write_message(lambda: record_xml_output(record),
                                  stream=sys.stderr)
                ...
                if callback_url:
                    ...
                    results_for_callback['results'].append({'recid': error[1], 'success': True, "marcxml": print_record(
                        error[1], 'xm'), 'url': "%s/%s/%s" % (CFG_SITE_URL, CFG_SITE_RECORD, error[1])})
            else:
                if callback_url:
                    results_for_callback['results'].append(
                        {'recid': error[1], 'success': False, 'error_message': error[2]})
    ...
    # Second phase -> Now we can process all entries where temporary
    # identifiers might appear (BDR, BDM)

    write_message(
        "Identifiers table after processing: %s  versions: %s" %
        (str(tmp_ids), str(tmp_vers)), verbose=2)
    write_message("Uploading BDR and BDM fields")
    if opt_mode != "holdingpen":
        for record in records:
            record_id=retrieve_rec_id(
                record, opt_mode, pretend=pretend, post_phase=True)
